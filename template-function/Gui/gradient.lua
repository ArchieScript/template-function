...
--============================================
    ...
    -- white ,black по отдельности не отключается, либо false,false, либо true,true
    -- при  white,black=true,true поведение яркости другое 
    -- https://avatars.mds.yandex.net/get-pdb/2129179/fd62e000-64bb-4994-8472-bc4bc6963663/s1200
    ...
    local function gradient(buf,vertically,x,y,w,h,white,black,alfa,col,brightness,frame);
        if h <= 5 then h = 0 end;
        if w <= 5 then w = 0 end;
        if white~=true or black~=true then white,black=false,false else white,black=true,true end
        buf = tonumber(buf)or 948;if buf<0 then buf=0 end;if buf>1023 then buf=1023 end;
        local b = brightness;
        alfa = tonumber(alfa)or 1;
        if alfa < 0 then alfa = 0 elseif alfa > 1 then alfa = 1 end;
        ...
        -----
        if white~=true and black~=true then;
        for i = 1,#col do;
            for c = 1,3 do;
                col[i][c] = col[i][c]+b;
                if col[i][c]>1 then col[i][c]=1 end;
