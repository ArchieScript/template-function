 ...
 -- "SkipNumberOfSamplesPerChannel = сколько сэмплов пропустить в секунду",
 -- samplerate это = сколько точек в секунде
 -- Устонавливать примерно "SkipNumberOfSamplesPerChannel" = samplerate / 100 - получется сто точек в секунду
 
 -- FeelVolumeOfItem = true  - Реагировать на громкость айтема
 -- FeelVolumeOfItem = false - Не реагировать на громкость айтема
...
при SampleRate (44100) деленном на сто, получается 441, это значет что функция будет прощитывать каждый 441 семпл, а т.е. (1,443,885,1327,1769 и т.д.), в итоге получится, что в секунду обработается сто семплов.

Все семплы в плюсе. Sample_min,Sample_max - это если айтем стерео или мультиканальный, то выбирается из всех каналов одно максимальное и одно минимальное значения семпла.
--]]
