...
----------------------
    ...
    -----------------------------------------
    path = path or ''
    scr_file = scr_file or 'CLEAN'
    local file = io.open(path..scr_file,'r');
    if not file then error("  There is no file !!! / Нет файла !!!  "..path..'/'..scr_file,1)end;
    ...
    ----------------------------------
     ...
     for var in string.gmatch(text,".")do;
         ...
         -------------------- 
         if var == "'" and two == 0 and not boxActiv then;
             if one > 0 then one = 0 else one = 1 end;
         end;
         if var == '"' and one == 0 and not boxActiv then;
             if two > 0 then two = 0 else two = 1 end;
         end;
         if var ~= '['then boxOpens = 0 end;
         if var ~= ']'then boxClose = 0 end;
         if t[i] == '[' and one == 0 and two == 0 and not boxActiv and #t2 < 2 then;
             boxOpens = boxOpens+1; if boxOpens >= 2 then;boxOpens = 0;
                 boxActiv = true;
         end;end;
         if t[i] == ']' and one == 0 and two == 0 and boxActiv then;
             boxClose = boxClose+1;if boxClose >= 2 then; boxClose = 0;
                  boxActiv = nil;
         end;end;
         ------------------------------
         t[#t+1] = var;
